---
title: "2017 Undiagnosed Estimates for WA State"
...
prev_path <- 'data/wa_prev_2010-2017.csv'
results_path <- 'wa/undx2017/results/undx2017.csv'
...
old_prev <- read.csv(file.path(undx_repo, 'data/wa_prev_2010-2016.csv'), 
                      check.names=FALSE,
                      header=TRUE)

# New 10/26/18 - last year's results
old_results <- read.csv(file.path(undx_repo, 'wa/undx2016/results/undx2016_formattedByHand.csv'),
                        header=TRUE, 
                        check.names=FALSE,
...
Going forward, there should be a section of the report devoted to comparing the latest dataset to the data provided in prior years on all the aforementioned measures. This report contains information on the change in PLWH data between the 2017 and 2018 years of estimation. 
...
kable(subset(wide, Year==2017, select=-Year), row.names=FALSE, booktabs=T) %>%
...
kable(subset(wide, Year==2016, select=-Year), row.names=FALSE, booktabs=T) %>%
      group_rows("By MSM", 2, 3) %>%
        group_rows("By KC", 4, 5)
```
...
As we would expect, the presence of observed LNTs is higher for MSM than non-MSM and inside KC vs outside KC. MSM outside of KC have much lower observed LNTs than MSM inside KC. This means that the outside-KC estimates, regardless of mode subgroup, are based on low levels of observed LNTs. This makes them particularly dependent on our 'missing at random' assumption.
...
Below, Current refers to the `r year_of_estimation` estimation. Prior refers to the `r year_of_estimation-1` estimates. 

```{r prev, plot=TRUE, fig.height=6, fig.cap="Absolute difference in HIV+ cases (top) and percent difference (bottom) between current and prior PLWH estimates, for 5 groups: Inside KC, Outside KC, MSM, non-MSM, and the Total. Note that Inside KC/Outside KC and MSM/non-MSM are separate marginal estimates."}
...
cprevw <- transform(cprevw, `Difference in Cases`=Current-Prior, 
                    ...
                    Group=variable,
                    check.names=FALSE)
cprevw <- subset(cprevw, Year<2017)
