---
title: "2017 Undiagnosed Estimates for King County - Darcy Version"
...
data_path <- 'data/kc_2018_Oct_formattedByApp.csv'
prev_path <- 'data/kc_2018_PLWHA.csv'
results_path <- 'kc/undx2017_Oct_Darcy/results/undx2017_Oct2018_Darcy.csv'
...
# If using mode3 for Darcy, need to scrap trying to include PLWH
#prev_data <- NULL
...
```{r mode3, echo=FALSE, include=FALSE}
# New for Darcy - run by mode3, not mode2, and exclude some categories
# Exclude 2=PWID, 7=Ped, and 8=Other
# Modes 1 & 3 are MSM+MSM/IDU
table(dataf$mode)
dataf <- subset(dataf, mode!=2 & mode!=7 & mode!=8)
dataf$mode3 <- 'MSM+MSM/IDU'
dataf <- within(dataf, {
                    mode3[mode==5] <- 'Hetero'
                    mode3[mode==9] <- 'NIR+NRR'
                      })
table(dataf$mode3)
dim(dataf)
dim(datafOrig)
...
kable(
      subset(wideall, Year>2015 & `Diagnoses/Case`=='Base Case', 
...
```{r , results='asis'}
kable(
      subset(wideallUB, Year>2015 & `Diagnoses/Case`=='Upper Bound', 
             select=-c(Year, `Diagnoses/Case`)
             ), row.names=FALSE, booktabs=T) %>%
# caption='In both tables, the MSM and KC results are separate marginal results, i.e. they are two different ways of dividing the statewide results.', booktabs=T) %>%
      group_rows("2016", 1, 3) %>%
