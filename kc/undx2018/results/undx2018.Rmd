---
title: "2018 Undiagnosed Estimates for King County"
...
data_path <- 'data/kc_2019_jbexport_formattedByApp.csv'
prev_path <- 'data/kc_2019_PLWHA.csv'
results_path <- 'kc/undx2018/results/undx2018.csv'
...
prev_data <- read.csv(file.path(undx_repo, prev_path), 
                      check.names=FALSE,
                      header=TRUE)
prev_data <- subset(prev_data, Year>=year_min)
```
...
## Base Case
```{r sumtable2018, results='asis'}
kable(
      subset(wideall, Year>2016 & `Diagnoses/Case`=='Base Case', 
...
```{r , results='asis'}
kable(
      subset(wideallUB, Year>2016 & `Diagnoses/Case`=='Upper Bound', 
             select=-c(Year, `Diagnoses/Case`)
             ), row.names=FALSE, booktabs=T) %>%
# caption='In both tables, the MSM and KC results are separate marginal results, i.e. they are two different ways of dividing the statewide results.', booktabs=T) %>%
      group_rows("2017", 1, 3) %>%
...
# Diagnoses
## Analytic sample
Analytic data set has `r nrow(dataf)` cases for 2012-2018. 
