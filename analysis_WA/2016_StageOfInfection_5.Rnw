...
This report differs from version 4 in that we now use the BED window of 162 days to define the infection window for BED+ cases with missing testing history (did not answer Yes or No to ``Have you ever had a negative test?"). Previously we left those infection windows as missing, effectively treating them as missing at random with respect to the stage of infection groups by which we stratified the estimation. This report also includes violin plots of the distributions of reported and imputed (for those reporting no testing history) infection windows. Additionally, the definition of dual diagnosis was corrected from using timeAids-timeHIVDx$<$1 year to using $\leq$1 year, which increased the \% with dual diagnoses from 33 to 34\%.
...
The graphs in Figure \ref{fig:DDtime} show how the percentages of cases with missing BED (left) and cases with dual diagnosis (right) vary over time. 

<<BEDmisstime, echo=FALSE, fig.pos='ht', fig.cap='Percent of cases who have a non-missing BED result, by year', fig.height=3, fig.width=5>>=
    ...
    stageTab <- ddply(dataf, c('yearDx'), function(x, TN=nrow(dataf)) {
      ...
      c(N=n,
        ...
        `Percent with BED`=round(100-100*sum(x$BED2fac=='BED Miss')/n,0),
        `Percent DD+`=round(100*sum(x$DD2=='DD +')/n,0))
    })

    sg <- ggplot(stageTab, aes(yearDx, `Percent with BED`)) + 
        ...
        ggtitle('% with a BED result')
@
<<DDtime, echo=FALSE, fig.pos='ht', fig.cap='Percent of cases who have non-missing BED (left) or a dual diagnoses (right), by year', fig.height=3, fig.width=7.5>>=

    dg <- ggplot(stageTab, aes(yearDx, `Percent DD+`)) + 
        stat_smooth(method='loess') + geom_point() +
        theme_bw() + 
        scale_y_continuous(limits=c(0,100), name='') +
        scale_x_continuous(name='', breaks=2005:2014) + 
        theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0.5)) +
        ggtitle('% with a dual diagnosis (DD+)')

    grid.arrange(sg,dg,ncol=2)
    #plot(dg)
@

\subsection{Monthly distribution of dual diagnoses}

<<DDqtr, echo=FALSE, fig.height=3, fig.width=4, fig.cap='Density of time lag between HIV and AIDS diagnoses among the dually diagnosed, measured in quarters (e.g, 4 quarters = 1 year)'>>=
    dd <- subset(dataf, dualDiag==TRUE)
    dd <- within(dd,{
                 ddQtr=AidsMinusHIVDx*4
    })
    ggplot(dd, aes(x=ddQtr, y=..density..)) + 
        geom_histogram(binwidth=1, origin=-0.5) +
        theme_bw() + 
        scale_x_continuous(name='Lag, # of quarters', breaks=0:4) + 
        ggtitle('Time lag between HIV and AIDS diagnoses\namong dual diagnoses')
@
...
The top bar plot in Figure \ref{fig:infWindows} shows the breakdown of the cases with non-missing testing history into those with a LNT and those who reported no LNT. For each of these groups, the lower violin plots show the distribution of the infection windows using a log scale. The violin plots have boxplots overlayed to show, when possible, the medians and interquartile ranges. The infection windows for those reporting no LNT come from the assumption $min(age-16,18)$. 

This plot shows that for the $>$75\% of cases with non-missing testing history who report a LNT, the median infection window is just over 1 year and about 25\% of cases are diagnosed within 6 mo nths of their LNT. For those reporting no LNT, at least half of them get the maximum infection window of 18 years.

Figure \ref{fig:infWindowsMSM} displays the same information stratified by MSM vs non-MSM. MSM (who comprise \Sexpr{round(100*c(table(dataf$mode2)/nrow(dataf))['MSM'],0)}\% of the sample) have a median infection window of 1 year when the LNT is reported. For non-MSM, the median infection window is over twice that. Non-MSM also have more than double the proportion of reports of no LNT. 

<<infWindows, echo=FALSE, fig.pos='h', fig.cap='', fig.height=6,fig.width=5, fig.cap='Distribution of infection windows among the 3013 cases with non-missing testing history'>>=
    dataf <- within(dataf, {
...
<<infWindowsMSM, echo=FALSE, fig.pos='h', fig.cap='Distribution of infection windows among the 3013 cases with non-missing testing history, stratified by MSM vs non-MSM'>>=
    grid.arrange(b+facet_grid(.~mode2),v+facet_grid(.~mode2),
                 heights=c(1.5,4))
@

There are three types of BED+ cases whose BED+ result influences our model: those who report having no LNT, those who report a LNT that is greater than the BED window (162 days), and those who have a missing answer to the testing history question. In all cases, we shorten the infection window to the BED window. 
...
rownames(cBEDdf) <- c('No LNT', 'LNT', 'Missing TH', 'Total')
...
We saw above that \Sexpr{cBEDdfall['Total', 4]}\% of all cases will be affected by the BED+ result. Of these \Sexpr{cBEDdfall['Total', 3]} cases, \Sexpr{cBEDdfall['LNT', 3]} have reported LNTs that will be shortened to the BED window, \Sexpr{cBEDdfall['No LNT', 3]} have assumed LNTs of $min(age-16,18)$ that will be shortened to the BED window, and \Sexpr{cBEDdfall['Missing TH', 3]} have missing testing history and will be given an infection window of the BED window. 

The infection windows of the BED+ before modification reflect the reported and assumed LNTs. When we perform the modifications specified above, we see that the long tail of infection windows is reigned in (Table \ref{tab:sumTable}). However, the median drops only slightly when the BED+ information is used, indicating only minor changes in the lower 50\% of the data. The first quartile increases due to the inclusion of cases with missing testing history who are given an infection window of the BED window.
...
The 1st quartile infection window increases slightly due to the use of the BED window for the \Sexpr{sumTable['Original BED+ infection windows',"NA's"]} missing testing histories that were given a BED window for their infection window.
...
\section{Appendix of Additional Results}

<<a1, echo=FALSE, results='asis'>>=
a1 <- with(dataf, table(mode2))
print(xtable(a1,
             caption='2005-2014 breakdown of MSM vs non-MSM diagnoses',
             label='tab:a1',
             digits=0),
      caption.placement='top',
      table.placement='ht',
...
<<a1fig, echo=FALSE, fig.pos='ht', fig.cap='MSM vs non-MSM diagnoses over time', fig.height=3, fig.width=7>>=
a1f <- as.data.frame(with(dataf, table(yearDx, mode2)))
ggplot(a1f, aes(yearDx, Freq, group=mode2)) + geom_point(aes(color=mode2)) + 
    geom_line(aes(color=mode2)) + theme_bw()
@

<<a2a, echo=FALSE, fig.pos='ht', fig.cap='Percent of cases who have a non-missing BED result, by year', fig.height=4, fig.width=7>>=

    stageTab2 <- ddply(dataf, c('yearDx', 'mode2'), function(x, TN=nrow(dataf)) {
      n <- nrow(x)
      c(N=n,
        `Column Percent`=round(100*n/TN,0),
        `Percent BED+`=round(100*sum(x$BED2fac=='BED +', na.rm=TRUE)/n,0),
    ...
    sg2 <- ggplot(stageTab2, aes(yearDx, `Percent with BED`, group=mode2)) + 
        stat_smooth(method='loess', aes(color=mode2)) + geom_point(aes(color=mode2)) +
        theme_bw() + scale_x_continuous(name='', breaks=2005:2014) + 
        scale_y_continuous(limits=c(0,100), name='') +
        theme(axis.text.x=element_text(angle=90,hjust=1,vjust=0.5)) + 
        ggtitle('% with a BED result')
    sg2
@
<<a2b, echo=FALSE, fig.pos='ht', fig.cap='Percent of cases who have a dual diagnoses, by year', fig.height=4, fig.width=7>>=

    dg2 <- ggplot(stageTab2, aes(yearDx, `Percent DD+`, group=mode2)) + 
        stat_smooth(method='loess', aes(color=mode2)) + geom_point(aes(color=mode2)) +
        theme_bw() + 
        scale_y_continuous(limits=c(0,100), name='') +
        scale_x_continuous(name='', breaks=2005:2014) + 
...
<<a3a, echo=FALSE, results='asis'>>=
a3a <- with(dataf, table(everHadNegTest, BED2fac, useNA='ifany'))
rownames(a3a) <- c('No LNT', 'LNT', 'Missing')
print(xtable(a3a,
             caption='LNT by BED',
             label='tab:a3a',
             digits=0),
      ...
      include.rownames=TRUE)
@
<<a3b, echo=FALSE, results='asis'>>=
a3b <- cbind(cBEDdf, rowSums(cBEDdf))
rownames(a3b) <- c('No LNT', 'LNT', 'Missing', 'Total')
colnames(a3b)[3]  <-  'Total'
print(xtable(a3b,
             caption='LNT by BED+ Impact',
             label='tab:a3b',
             digits=0),
      caption.placement='top',
      table.placement='ht',
      ...
      include.rownames=TRUE)
@
<<a4a, echo=FALSE, fig.pos='h', fig.cap='', fig.height=6,fig.width=5, fig.cap='Distribution of infection windows among the BED+ (Original N=691, Modified N=880)'>>=
    datafBED <- subset(dataf, BED2fac=='BED +')
    datafBEDlong <- rbind(datafBED, datafBED)
    datafBEDlong <- transform(datafBEDlong,
                              WindowType=c(rep('Original', nrow(datafBED)),
    ...
    a4a <- ggplot(subset(datafBEDlong,!is.na(LNT)), 
                aes(WindowType, infWindow))
    a4a <- a4a + geom_violin(width=1,  aes(fill=WindowType), alpha=0.5) + 
        scale_y_log10(breaks=c(0.25,0.5,1,2,5,18), name='Infection Window, Years') + 
        geom_boxplot(width=0.05, outlier.size=0, 
                     aes(fill=WindowType)) + 
        coord_flip() + 
...
#        facet_grid(BED2fac~.) + 
        ...
        ggtitle('Among the BED+, original vs modified infection windows')
    a4a
@
<<a4b, echo=FALSE, fig.pos='h', fig.cap='', fig.height=6,fig.width=7, fig.cap='Distribution of infection windows among the BED+ (Original N=691, Modified N=880), by MSM vs non-MSM'>>=
    a4a + facet_grid(.~mode2)
@
\end{document}
